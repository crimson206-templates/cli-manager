...
class AddCommand(Command):
    ...
    arguments = [
        argument(
            "cli_names",
            "Names of CLIs to add (space-separated)",
            multiple=True
        )
    ]
    ...
    options = [
        option(
            "force",
            "f",
            "Force add even if CLI already exists",
            flag=True
        )
    ]
    ...
    def handle(self) -> int:
        ...
        for cli_name in cli_names:
            ...
            if cli_name in registered_clis and not force:
                self.line(f"<error>CLI '{cli_name}' is already registered. Use --force to override.</error>")
            ...
            try:
                ...
                if not completion_success:
                    self.line(f"<error>Failed to update completion for '{cli_name}': {messages[0]}</error>")
                    failed_clis.append(cli_name)
                    continue
        ...
        if failed_clis:
            self.line(f"<error>Failed to add {len(failed_clis)} CLI(s): {', '.join(failed_clis)}</error>")
            return 1
